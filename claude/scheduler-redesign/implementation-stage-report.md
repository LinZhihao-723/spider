...
| `a_slow_resource_group_neither_blocks_nor_takes_over_the_buffer` | 4 RG, RG0's worker at 50 ms/task vs. 1 ms, `B = 32` | Exactly-once; pinned isolation; every worker received its full workload; longest uninterrupted RG0 mint run ≤ `B/2` |
...
**`a_slow_resource_group_neither_blocks_nor_takes_over_the_buffer` is the weakest test, and it guards the most important property.**

- The occupancy bound is asserted through a **proxy**, not a measurement. The test reconstructs the order in which assignments were minted and bounds the longest run of consecutive mints belonging to the slow group. The inference — a run of `n` mints with no other group served drives that group's occupancy to nearly `n`, because its worker consumes at most one assignment per 50 ms against a 1 ms tick — is sound but indirect. Queue occupancy is never read.
- The bound used is `B/2`, the **lone-group** ceiling from §6.3. With four groups backlogged the design predicts the slow group creeps toward `B/(N+1) ≈ 6`. The assertion would pass with it at 15 — 2.4× its equilibrium share. The tight bound is asserted only in the single-tick, in-process unit test.
- **"Does not block the others" is not asserted at all.** The test asserts each worker *completed* its workload, which is liveness. There is no rate, no time-to-completion, and no comparison against a baseline without the slow group. §1.1's non-interference guarantee is therefore not measured end to end.
